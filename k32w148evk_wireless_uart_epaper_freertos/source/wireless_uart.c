...
OSA_EVENT_HANDLE_DEFINE(mNewPriceEvent);
messaging_t mPriceInputQueue;
...
static TIMER_MANAGER_HANDLE_DEFINE(mSwitchPressTimer);
static uint8_t mSwitchPressTimerId;
#endif
...
)
{
    switch (pConnectionEvent->eventType)
    {
        ...
        case gConnEvtDisconnected_c:
        {
            ...
            Serial_PrintDec(peerDeviceId);
            Serial_Print(".\n\r", gAllowToBlock_d);
            Serial_Print("Reason :", gAllowToBlock_d);
            Serial_PrintHex(pConnectionEvent->eventData.disconnectedEvent.reason >> 8);
            Serial_PrintHex(pConnectionEvent->eventData.disconnectedEvent.reason);
            Serial_Print(".\n\r", gAllowToBlock_d);
...
static void BleApp_SendPriceData
(
	uint8_t *pStream,
	uint16_t streamLength
)
{
	char* pPriceCommand = "PX";
	if (streamLength == (18 + strlen(pPriceCommand)) && memcmp(pStream, pPriceCommand, strlen(pPriceCommand)) == 0)
	{
		Serial_Print("Found price information\r\n", gAllowToBlock_d);
		uint8_t valuesAsIntegers[18];
		for (int i = 0 ; i < (streamLength - strlen(pPriceCommand)) ; i++)
		{
			valuesAsIntegers[i] = pStream[i+strlen(pPriceCommand)] - 48;
		}
		appPriceInformation_t *priceMsg = NULL;
		priceMsg = MSG_Alloc(sizeof(appPriceInformation_t));
		memcpy(priceMsg, valuesAsIntegers, 18);

		Serial_Print("Currency :", gAllowToBlock_d);
		Serial_PrintDec(priceMsg->currency);
		Serial_Print("\r\n", gAllowToBlock_d);
		Serial_Print("Price :", gAllowToBlock_d);
		Serial_PrintDec(priceMsg->tens);
		Serial_PrintDec(priceMsg->units);
		Serial_Print(".", gAllowToBlock_d);
		Serial_PrintDec(priceMsg->tenths);
		Serial_PrintDec(priceMsg->cents);
		Serial_Print("\r\n", gAllowToBlock_d);

	    /* Put message in the Host Stack to App queue */
	    (void)MSG_QueueAddTail(&mPriceInputQueue, priceMsg);

	    /* Signal application */
	    (void)OSA_EventSet(mNewPriceEvent, 1);
...
)
{
    ...
    BleApp_SendPriceData(pStream, streamLength);
...
#if (gAppButtonCnt_c == 1)
    (void)TM_Open(mSwitchPressTimer);
